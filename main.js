...
const xml2js = require('xml2js'); // node-Modul um xml Strukturen in JSON umzuwandeln. Eine Beschreibung: http://www.flyacts.com/blog/nodejs-xml-daten-verarbeiten/
...
var configDeltaTimeOKSec = 5, // Abweichung in Sekunden zwischen der Systemzeit von ioBroker und der Meldungszeit aus der Fritzbox, die noch als OK gilt
...
adapter.on('message', function (_obj) {
...
adapter.on('stateChange', function (id, state) {
    ...
    if (id === `${adapter.namespace}.calls.missedCount`) {
        ...
        if (state.val != wlanState && intervalTR046 && adapter.config.enableWlan) {
            ...
            setWlanEnabled(
                adapter.config.fritzboxAddress,
                adapter.config.fritzboxUser,
                adapter.config.fritzboxPassword,
                state.val,
            );
        }
    }
...
function numberFormat(number, length, align) {
    ...
    if (!number.match(onlyNumbers)) {
        // wenn der String numbers nur Ziffern enthält und diese in der Anzahl zu lang sind, kürzen und die letzte Ziffer gegen ein "x" ersetzen
    ...
    while (number.search(' ') >= 0) {
        ...
        if (!i) {
            adapter.log.debug(`Leerzeichen gegen utf-8 non breaking space ersetzt: <${number}>`);
        }
        i++;
        if (i > 40) {
...
function fritzboxDateToTableDate(fritzboxDate) {
    ...
    var date = `${day}.${month}.${nbsp}${hour}:${minute}${nbsp}${nbsp}`; // space = non-breaking-space in utf-8
    return date;
}
...
function callmonitor(list) {
    ...
    for (var i = 0; i < list.length; i++) {
        txt += `${fritzboxDateToTableDate(call[list[i].id].dateStart)}${nbsp}${call[list[i].id].externalNumberForm}`;
...
function callmonitorAll(list) {
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        txt += `${fritzboxDateToTableDate(call[id].dateStart)}${nbsp}${call[id].externalNumberForm}`;
        txt += `${call[id].callSymbolColor}${nbsp}`;
        txt += intNr;
        if (call[id].type === 'CONNECT') {
            txt += `${nbsp}<span style="color:green"><b>${durationForm(call[id].durationSecs2)}</b></span>`;
        }
        if (call[id].type === 'RING') {
            txt += `${nbsp}<span style="color:red  ">${durationForm(call[id].durationRingSecs)}</span>`;
        }
        if (i < list.length - 1) {
            txt += txt ? '<br>\n' : '';
        }
...
function headlineMissedHTML() {
    var headlineHistoryMissedHTML = `<b>${headlineDate}${headlineExtnumber}${nbsp}${headlineOwnnumber}</b>`;
    ...
    adapter.getState(object, function (err, state) {
        if (!err && state) {
            ...
            if (state.val == newValue) {
                return;
            } // vorhandene Werte nicht noch einmal schreiben
...
function parseData(message) {
    ...
    } else if (call[id].type === 'DISCONNECT') {
        // End of call
        // End of call
    ...
    if (
        call[id].externalNumber === '' ||
        call[id].externalNumber === null ||
        call[id].externalNumber === configUnknownNumber ||
        call[id].externalNumber === '????????'
    ) {
        if (call[id].externalNumber !== '????????') {
            call[id].externalNumber = configUnknownNumber;
        }
...
*/
    ...
    adapter.log.debug(
        `ringCount: ${ringCount}, callCount: ${callCount}, connectCount: ${connectCount}, allActiveCount: ${allActiveCount}`,
    );
    ...
    if (call[id].type === 'DISCONNECT') {
        if (call[id].direction === 'in') {
            ...
            if (!call[id].connect) {
                // letzter Anrufer, der verpasst wurde
    ...
    if (!call[id].deltaTimeOK) {
        adapter.log.warn(`delta time between system and fritzbox: ${call[id].deltaTime} sec`);
    }

    //Auf Änderungen im ioBroker Objekt reagieren und die lokale Variable auch ändern.
    if (objMissedCount !== missedCount) {
        // Zähler nur schreiben, wenn er sich verändert hat (wg. Traffic Überwachung des Objekts auf Änderung)
    ...
    if (call[id].type === 'DISCONNECT') {
        ...
        var lineHistoryAllTxt =
            ...
            call[id].externalNumberForm +
            nbsp +
            call[id].callSymbol +
            extensionLine +
            nbsp +
            call[id].ownNumberForm +
            nbsp +
            call[id].lineType +
            nbsp +
        ...
        if (!call[id].connect) {
            if (call[id].direction === 'in') {
                ...
                var lineHistoryMissedHtml =
                    fritzboxDateToTableDate(call[id].date) + externalNumber + nbsp + call[id].ownNumberForm + nbsp;
        ...
        if (showMissedTableHTML) {
            ...
            if (!call[id].connect) {
                if (call[id].direction === 'in') {
                    var historyListMissedHtmlStr = makeList(
                        historyListMissedHtml,
                        lineHistoryMissedHtml,
                        headlineTableMissedHTML,
                        configHistoryMissedLines,
                        configShowHeadline,
                    );
        ...
        if (showHistoryAllTableHTML) {
            ...
            var historyListAllHtmlStr = makeList(
                historyListAllHtml,
                lineHistoryAllHtml,
                headlineTableAllHTML,
                configHistoryAllLines,
                configShowHeadline,
            );
        ...
        if (showHistoryAllTableTxt) {
            ...
            var historyListAllTxtStr = makeList(
                historyListAllTxt,
                lineHistoryAllTxt,
                headlineTableAllTxt,
                configHistoryAllLines,
        ...
        if (allActiveCount && !intervalRunningCall) {
            ...
            intervalRunningCall = setInterval(function () {
                ...
                for (var i = 0; i < listConnect.length; i++) {
                    call[listConnect[i].id].durationSecs2 =
                        dateEpochNow() / 1000 - call[listConnect[i].id].dateEpochNow / 1000;
                }
                for (i = 0; i < listRing.length; i++) {
                    call[listRing[i].id].durationRingSecs =
                        dateEpochNow() / 1000 - call[listRing[i].id].dateEpochNow / 1000;
                }
...
function setWlanEnabled(host, user, password, enabled) {
    connectToTR064(host, user, password, function (sslDev) {
        ...
        wlanConfig.actions.SetEnable({ NewEnable: enabled ? 1 : 0 }, function (err, _result) {
            if (!err) {
                ...
                request(
                    ...
                    function (error, response, body) {
                        if (!error && response.statusCode == 200) {
                            ...
                            parser.parseString(body, function (err, result) {
                                if (err) {
                                    ...
                                    if (!result.Root.Message) {
                                        // No messahes
                                        promises.push(new Promise((resolve, _reject) => resolve()));
                                    } else {
                                            ...
                                            if (typeof message != 'undefined') {
                                                promises.push(
                                                    new Promise((resolve, _reject) => {
                                                        ...
                                                        adapter.log.debug(
                                                            `TR-064: Download TAM audio file from ${downloadUrl}`,
                                                        );

                                                        const stream = createWriteStream(file);
                                                        stream.on('error', err => {
                                                            adapter.log.error(
                                                                `Could not write tam file: ${err.message}`,
                                                            );
                                                        });
                                    ...
                                    Promise.all(promises).then(function () {
                                        ...
                                        readdir('tam', (err, files) => {
                                            if (err) {
                                                adapter.log.warn(`TR-064: Error reading files from dir /tam: ${err}`);
                                            } else {
                                                files.forEach(file => {
                                                    file = path.resolve(`tam/${file}`);
                                                    if (!messages.find(msg => msg.audioFile == file)) {
                                                        // old file
                                                        adapter.log.debug(`TR-064: Remove old tam audio file: ${file}`);
                            ...
                            adapter.log.warn(`TR-064: Error while requesting TAM: ${error}`);
                        }
                    },
                );
            }
                        ...
                        if (!error && response.statusCode == 200) {
                            ...
                            parser.parseString(body, function (err, result) {
                                if (err) {
                                    ...
                                    adapter.log.debug(
                                        'TR-064: Successfully parsed phonebook content, analyzing result ...',
                                    );
                                    ...
                                    for (var c = 0; c <= phonebook.contact.length; c++) {
                                        ...
                                        if (typeof contact !== 'undefined') {
                                            ...
                                            for (var t = 0; t <= contact.telephony.length; t++) {
                                                ...
                                                if (
                                                    typeof telephony !== 'undefined' &&
                                                    typeof telephony.number !== 'undefined'
                                                ) {
                                                    for (var n = 0; n <= telephony.number.length; n++) {
                                                        ...
                                                        if (typeof number != 'undefined') {
                                                            ...
                                                            if (
                                                                entryNumber.startsWith('0') ||
                                                                entryNumber.startsWith('+')
                                                            ) {
                                                                phonenumbers.push({
                                                                    key: entryNumber,
                            ...
                            adapter.log.warn(`TR-064: Error while requesting phonebook: ${error}`);
                        }
                    },
                );
            }
...
function connectToFritzbox(host) {
    ...
    if (
        (adapter.config.enableWlan || adapter.config.enablePhonebook || adapter.config.enableTAM) &&
        adapter.config.fritzboxUser &&
        adapter.config.fritzboxPassword &&
        adapter.config.fritzboxPassword.length
    ) {
...
function main() {
    ...
    instanceDir = utils.getAbsoluteInstanceDataDir(adapter);
    try {
